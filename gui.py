...
class ControleVacinas:
    ...
    TEMAS = {
        ...
        "SUS (Azul)": {
            ...
            "fg_lista": "#333333",
            "bg_campos": "#FFFFFF",
            "botao_principal": "#1976D2",
            "botao_sucesso": "#388E3C",
            "botao_alerta": "#D32F2F",
            "botao_info": "#F57C00",
            "botao_entrada": "#1565C0",
            "botao_saida": "#F57C00",
        },
        "Verde/Água": {
            "bg_janela": "#E0F2F1",
            ...
            "fg_lista": "#004D40",
            "bg_campos": "#FFFFFF",
            "botao_principal": "#00796B",
            "botao_sucesso": "#2E7D32",
            "botao_alerta": "#C62828",
            "botao_info": "#FF8F00",
            "botao_entrada": "#00838F",
            "botao_saida": "#FF6F00",
        },
        "Rosa/Vermelho": {
            "bg_janela": "#FCE4EC",
            ...
            "fg_lista": "#880E4F",
            "bg_campos": "#FFFFFF",
            "botao_principal": "#AD1457",
            "botao_sucesso": "#C62828",
            "botao_alerta": "#D32F2F",
            "botao_info": "#E91E63",
            "botao_entrada": "#880E4F",
            "botao_saida": "#D32F2F",
        },
        "Escuro (Modo Noturno)": {
            "bg_janela": "#1E1E1E",
            ...
            "fg_lista": "#E0E0E0",
            "bg_campos": "#2D2D2D",
            "botao_principal": "#BB86FC",
            "botao_sucesso": "#03DAC6",
            "botao_alerta": "#CF6679",
            "botao_info": "#FFB74D",
            "botao_entrada": "#BB86FC",
            "botao_saida": "#CF6679",
        },
        "Laranja/Dourado": {
            "bg_janela": "#FFF8E1",
        ...
        "Amarelo/Dourado": {
            ...
            "fg_lista": "#333333",
            "bg_campos": "#FFFFFF",
            "botao_principal": "#F9A825",
            "botao_sucesso": "#43A047",
            "botao_alerta": "#E53935",
            "botao_info": "#FB8C00",
            "botao_entrada": "#00897B",
            "botao_saida": "#F57C00",
        },
        ...
        "Pastel": {
            ...
            "fg_lista": "#4A148C",
            "bg_campos": "#FFFFFF",
            "botao_principal": "#AB47BC",
            "botao_sucesso": "#66BB6A",
            "botao_alerta": "#EF5350",
            "botao_info": "#FFA726",
            "botao_entrada": "#26A69A",
            "botao_saida": "#FF7043",
        },
            }
    ...
    def aplicar_tema(self, tema_nome):
        """Aplica um tema ao sistema todo"""
        ...
        # Aplica cor na janela principal
        ...
        # Atualiza o combobox se existir
        if hasattr(self, 'tema_combo'):
            self.tema_combo.set(tema_nome)
    
    def carregar_dados(self):
        """Carrega os dados com migração automática para o novo formato"""
        try:
            if os.path.exists(ARQUIVO):
                ...
                if dados and isinstance(dados, dict):
                    ...
                    primeiro_valor = dados[primeira_chave]
                    
                    if isinstance(primeiro_valor, str):
                        print("Migrando dados do formato antigo...")
                        novos_dados = {}
                        hoje = datetime.now()
                        for nome, lote in dados.items():
                            id_lote = self.gerar_id(nome, lote)
                            novos_dados[id_lote] = {
                                "nome": nome,
                                "lote": lote,
                                "quantidade": 100,
                    ...
                    migrou = False
                    hoje = datetime.now()
                    for id_lote, info in dados.items():
                        if isinstance(info, dict):
                            if "validade" not in info:
                                info["validade"] = (hoje + timedelta(days=365)).strftime("%d/%m/%Y")
                                migrou = True
                            if "em_uso" not in info:
                                info["em_uso"] = False
                                migrou = True
                            if "minimo" not in info:
                                info["minimo"] = 30
                                migrou = True
                    if migrou:
                        with open(ARQUIVO, "w", encoding="utf-8") as f:
    ...
    def criar_autocomplete(self, entry, lista_completa, is_principal=True):
        """Cria uma lista flutuante de autocomplete para o entry
        is_principal: True para tela principal (preenche lote), False para outras telas
        """
        ...
        def selecionar(event=None):
            if listbox.curselection():
                ...
                # Se for a tela principal, preenche o lote automaticamente
                if is_principal:
                    # Procura a vacina que está EM USO
    ...
    def atualizar_sugestoes_principal(self):
        if hasattr(self, 'lista_sugestoes_principal'):
           self.lista_sugestoes_est = self.criar_autocomplete(self.entrada_nome_est, list(self.vacinas.keys()), is_principal=False)
    ...
    def criar_interface(self):
        ...
        tk.Label(campos, text="💊 Nome da Vacina:", font=("Arial", 11, "bold"), bg=tema["bg_campos"]).pack(anchor="w", padx=5)
        ...
        self.lista_sugestoes_principal = self.criar_autocomplete(self.entrada_nome_principal, self.get_nomes_em_uso())
        
        tk.Label(campos, text="🔢 Lote:", font=("Arial", 11, "bold"), bg=tema["bg_campos"]).pack(anchor="w", padx=5)
        ...
        tk.Label(campos_est, text="💊 Vacina:", font=("Arial", 10, "bold"), bg=tema["bg_campos"]).pack(anchor="w", padx=5)
        ...
        tk.Label(campos_est, text="🔢 Lote:", font=("Arial", 10, "bold"), bg=tema["bg_campos"]).pack(anchor="w", padx=5)
        ...
        tk.Label(campos_est, text="📅 Validade (DD/MM/AAAA):", font=("Arial", 10, "bold"), bg=tema["bg_campos"]).pack(anchor="w", padx=5)
        ...
        tk.Label(qtde_frame, text="📊 Quantidade:", font=("Arial", 10, "bold"), bg=tema["bg_campos"]).pack(side="left")
        ...
        tk.Label(qtde_frame, text="⚠️ Mínimo:", font=("Arial", 10, "bold"), bg=tema["bg_campos"]).pack(side="left", padx=(20, 0))
...
📁 Usuário > SistemaVacinas
        ...
        # Seletor de temas
        tk.Label(frame_sobre, text="🎨 ESCOLHA UM TEMA:", font=("Arial", 12, "bold"), bg="white", fg="#1976D2").pack(pady=(10, 5))
        
        self.tema_combo = ttk.Combobox(frame_sobre, textvariable=tk.StringVar(value=self.tema_atual), 
                                        values=list(self.TEMAS.keys()), font=("Arial", 11), state="readonly", width=20)
        self.tema_combo.pack(pady=5)
        
        def mudar_tema():
            self.aplicar_tema(self.tema_combo.get())
        
        tk.Button(frame_sobre, text="🎨 APLICAR TEMA", font=("Arial", 11, "bold"), 
                  bg="#1976D2", fg="white", command=mudar_tema, height=1, padx=20).pack(pady=10)
        
        # Rodapé alertas
        alerta_frame = tk.Frame(self.janela, bg="#FFF3E0", relief="ridge", bd=2)
        alerta_frame.pack(fill="x", padx=10, pady=(0, 10))
        self.alerta_label = tk.Label(alerta_frame, text="✅ Sistema OK", font=("Arial", 10), bg="#FFF3E0", fg="#E65100", wraplength=550)
    ...
    def atualizar_lista_principal(self):
        ...
        for i, dados in enumerate(sorted(em_uso, key=lambda x: x["nome"])):
            ...
            self.lista_principal.itemconfig(i, bg=cor_fundo, fg="#333")
    ...
    def atualizar_lista_estoque(self):
        ...
        for i, (id_lote, dados) in enumerate(sorted(itens_validos, key=lambda x: x[1]["nome"])):
            ...
            validade_status, _ = self.verificar_validade(dados.get("validade", ""))
            texto = f"{dados['nome']}   |   Lote: {dados['lote']}   |   Estoque: {dados['quantidade']}   |   {validade_status}   |   {status}"
            ...
            if "VENCIDA" in validade_status:
                ...
                self.lista_estoque.itemconfig(i, fg="#1976D2")
            else:
                self.lista_estoque.itemconfig(i, fg="#666")
    ...
    def adicionar_lote(self):
        ...
        self.salvar_historico("ADICIONAR LOTE", nome, lote, quantidade, f"Validade: {validade}")
        self.salvar_dados()
        self.atualizar_lista_estoque()
    ...
    def atualizar_dados_lote(self):
        ...
        self.salvar_historico("ATUALIZAR DADOS", dados["nome"], dados["lote"], nova_qtde, f"Minimo: {novo_minimo}, Validade: {nova_validade}")
        self.salvar_dados()
        self.atualizar_lista_principal()
        self.atualizar_lista_estoque()
